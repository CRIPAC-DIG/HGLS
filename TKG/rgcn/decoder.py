...
class ConvTransR(torch.nn.Module):
    ...
    def forward(self, embedding, emb_rel, embedding_o, triplets, nodes_id=None, mode="train", negative_rate=0):

        e1_embedded_all = torch.tanh(embedding)
        e1_embedded_o_all = torch.tanh(embedding_o)
        batch_size = len(triplets)
        # if mode=="train":
        e1_embedded = e1_embedded_all[triplets[:, 0]].unsqueeze(1)
        e2_embedded = e1_embedded_o_all[triplets[:, 2]].unsqueeze(1)
        # else:
        #     e1_embedded = e1_embedded_all[triplets[:, 0]].unsqueeze(1)
...
class ConvTransE(torch.nn.Module):
    ...
    def forward(self, embedding, emb_rel, embedding_o, triplets, pos=None, neg=None, mode='train'):
        e1_embedded_all = torch.tanh(embedding)
        embedding_o = torch.tanh(embedding_o)
        batch_size = len(triplets)
        e1_embedded = e1_embedded_all[triplets[:, 0]].unsqueeze(1)
        ...
        if mode == 'train':
            if pos is not None and neg is not None:
                score_pos = (x*embedding_o[pos]).sum(1).unsqueeze(1)
                score_neg = torch.matmul(x.unsqueeze(1), embedding_o[neg].transpose(2,1)).squeeze(1)
                score = -(torch.log(torch.sigmoid(score_pos)) + torch.log(1-torch.sigmoid(score_neg))).sum()/score_pos.shape[0]
                return score
            else:
                #x = torch.mm(x, e1_embedded_all.transpose(1, 0))
                x = torch.mm(x, embedding_o.transpose(1, 0))
            return x
        elif mode == 'test':
            # x = torch.mm(x, e1_embedded_all.transpose(1, 0))
            x = torch.mm(x, embedding_o.transpose(1, 0))
